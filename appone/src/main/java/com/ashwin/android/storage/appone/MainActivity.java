package com.ashwin.android.storage.appone;
...
public class MainActivity extends AppCompatActivity {
    public static final String TAG = "app-one";
    ...
    public void read(View view) {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                if (cursor.getCount() == 0) {
                    Log.e(TAG, "read | No record found");
                } else {
                    Uri uri = null;
                    while (cursor.moveToNext()) {
                        String fileName = cursor.getString(cursor.getColumnIndex(MediaStore.MediaColumns.DISPLAY_NAME));
                        String relativePath = cursor.getString(cursor.getColumnIndex(MediaStore.MediaColumns.RELATIVE_PATH));
                        Log.w(TAG, "read | fileName: " + fileName + ", relativePath: " + relativePath);
                        long id = cursor.getLong(cursor.getColumnIndex(MediaStore.MediaColumns._ID));
                        uri = ContentUris.withAppendedId(contentUri, id);
                        break;
                    }
                    ...
                    if (uri == null) {
                        Log.e(TAG, "read | File NOT found");
                    } else {
                        Log.w(TAG, "read | uri: " + uri);  // E.g. content://media/external/file/1022
                        ...
                        /*ParcelFileDescriptor parcelFileDescriptor = getContentResolver().openFile(uri, "r", null);
    ...
    public void delete(View view) {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                ...
                String selection = MediaStore.MediaColumns.RELATIVE_PATH + " = ? AND " + MediaStore.MediaColumns.DISPLAY_NAME + " = ?";
                String[] selectionArgs = new String[]{getDirName() + "/", getFileName()};
                int result = getContentResolver().delete(contentUri, selection, selectionArgs);
